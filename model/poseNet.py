...
class poseNet(LightningModule):
    ...
    def __init__(self, nstack, inp_dim, oup_dim, batch_size,bn=False, **kwargs):
        ...
        self.pred_heatmaps = nn.ModuleList([
            Conv(inp_dim, oup_dim, 1, bn=False, relu=False) for num in range(nstack)
        ])

        self.module_outs = nn.ModuleList([
            Conv(inp_dim, inp_dim, 1, bn=False, relu=False) for num in range(nstack-1)
        ])
