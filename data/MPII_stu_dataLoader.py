...
class MPIIDataLoader(torch.utils.data.Dataset):
    def __init__(self, config, mpii, index, pred_tch):
    ...
    def __getitem__(self, idx):
        img, hmp = self.loadImageAndGt(self.index[idx  % len(self.index)])
        #print('len ', len(self.pred_tch))
        #print('idx ', idx)
        return img, hmp,\
               self.pred_tch[self.index[idx % len(self.index)]].astype(np.float32)
...
def init(config):
    ...
    print('loading tch prediction...')
    pred_tch = np.load(file='./data/pred_tch.npy')
    ...
    trainset = MPIIDataLoader(config, mpii, train_index, pred_tch)
    validset = MPIIDataLoader(config, mpii, valid_index, pred_tch)

    mpii_which_dataloader = DataLoader(train_index, batch_size=8, num_workers=8,
                                       shuffle=False)
